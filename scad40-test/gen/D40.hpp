...
namespace glfw3 {
	...
	/// a global class
	struct Sys : public scad40::_object
	{
		/// the Sys constructor
		/// ... the user must implement this
		Sys(void);
	...
	inline void install(duk_context* ctx)
	{
		...
		{
			// stack -> .... base .. ;
			// peterlavalle.glfw3/Sys
			{
				duk_push_object(ctx);
				// stack -> .... base .. ; [Sys] ;

				peterlavalle::glfw3::Sys* thisSys = (peterlavalle::glfw3::Sys*) duk_alloc(ctx, sizeof(peterlavalle::glfw3::Sys));
				duk_push_pointer(ctx, thisSys);
				// stack -> .... base .. ; [Sys] ; *Sys ;

				duk_put_prop_string(ctx, idxBase, "\xFF" "*Sys");
				// stack -> .... base .. ; [Sys] ;

				scad40::push_selfie<peterlavalle::glfw3::Sys>(ctx, thisSys, 0, [](duk_context* ctx, peterlavalle::glfw3::Sys* thisSys) -> duk_ret_t {

					assert(ctx == thisSys->Host());

					thisSys->~Sys();
					duk_free(ctx, thisSys);
					// scad40::env::remove(ctx, "peterlavalle.glfw3.Sys");
					// TODO ; find a way to actually DO THIS!?!?
				...
				// def open(title: string, listener: string): void
					scad40::push_selfie<peterlavalle::glfw3::Sys>(ctx, thisSys, 2, [](duk_context* ctx, peterlavalle::glfw3::Sys* thisSys) -> duk_ret_t {
						thisSys->open(
							scad40::duk_string(ctx, 0),
				...
				scad40::env::assign(ctx, "peterlavalle.glfw3.Sys");
				// stack -> .... base .. ;

				*reinterpret_cast<duk_context**>(thisSys) = ctx;
				new (thisSys) peterlavalle::glfw3::Sys();
...
#pragma region "global Sys"
inline peterlavalle::glfw3::Sys& peterlavalle::glfw3::Sys::get(duk_context* ctx)
{
	auto base = duk_get_top(ctx);

	// stack -> .... base .. ;

	scad40::env::lookup(ctx, "peterlavalle.glfw3.Sys");
	// stack -> .... base .. ; [Sys] ;

	duk_get_prop_string(ctx, base, "\xFF" "*Sys");
	// stack -> .... base .. ; [Sys] ; Sys[*] ;

	auto ptrSys = reinterpret_cast<peterlavalle::glfw3::Sys*>(duk_to_pointer(ctx, -1));
	duk_pop_2(ctx);
	// stack -> .... base .. ;
