...
namespace magpie {
	...
	/// a global class
	struct Time : public scad40::_object
	{
		...
		/// locates the singleton instance
		static Time& get(duk_context*);
	};

	/// a global class
	struct System : public scad40::_object
	{
		...
		/// the user's requested members
		/// ... the user must implement these
			bool require (scad40::duk_string& name);
			void out (scad40::duk_string& message);
			void err (scad40::duk_string& message);

		/// alternative const char* interfaces
			inline bool require (const char* name)
			{
				return require(
	...
	inline void install(duk_context* ctx)
	{
		...
		{
			// stack -> .... base .. ;
			// peterlavalle.magpie/Time
			{
				duk_push_object(ctx);
				// stack -> .... base .. ; [Time] ;

				peterlavalle::magpie::Time* thisTime = (peterlavalle::magpie::Time*) duk_alloc(ctx, sizeof(peterlavalle::magpie::Time));
				duk_push_pointer(ctx, thisTime);
				// stack -> .... base .. ; [Time] ; *Time ;

				duk_put_prop_string(ctx, idxBase, "\xFF" "*Time");
				// stack -> .... base .. ; [Time] ;

				scad40::push_selfie<peterlavalle::magpie::Time>(ctx, thisTime, 0, [](duk_context* ctx, peterlavalle::magpie::Time* thisTime) -> duk_ret_t {

					assert(ctx == thisTime->Host());

					thisTime->~Time();
					duk_free(ctx, thisTime);
					// scad40::env::remove(ctx, "peterlavalle.magpie.Time");
					// TODO ; find a way to actually DO THIS!?!?
				...
				// val now: single
					duk_push_string(ctx, "now");
					scad40::push_selfie<peterlavalle::magpie::Time>(ctx, thisTime, 3, [](duk_context* ctx, peterlavalle::magpie::Time* thisTime) -> duk_ret_t {
						duk_push_number(ctx, thisTime->_now);
						return 1;
					});
					duk_def_prop(ctx, idxBase, DUK_DEFPROP_HAVE_GETTER | DUK_DEFPROP_HAVE_ENUMERABLE);

				// val delta: single
					duk_push_string(ctx, "delta");
					scad40::push_selfie<peterlavalle::magpie::Time>(ctx, thisTime, 3, [](duk_context* ctx, peterlavalle::magpie::Time* thisTime) -> duk_ret_t {
						duk_push_number(ctx, thisTime->_delta);
						return 1;
					});
					duk_def_prop(ctx, idxBase, DUK_DEFPROP_HAVE_GETTER | DUK_DEFPROP_HAVE_ENUMERABLE);
				...
				scad40::env::assign(ctx, "peterlavalle.magpie.Time");
				// stack -> .... base .. ;

				*reinterpret_cast<duk_context**>(thisTime) = ctx;
				new (thisTime) peterlavalle::magpie::Time();

				assert(duk_get_top(ctx) == idxBase);
			}
			// peterlavalle.magpie/System
			{
				duk_push_object(ctx);
				// stack -> .... base .. ; [System] ;

				peterlavalle::magpie::System* thisSystem = (peterlavalle::magpie::System*) duk_alloc(ctx, sizeof(peterlavalle::magpie::System));
				duk_push_pointer(ctx, thisSystem);
				// stack -> .... base .. ; [System] ; *System ;

				duk_put_prop_string(ctx, idxBase, "\xFF" "*System");
				// stack -> .... base .. ; [System] ;

				scad40::push_selfie<peterlavalle::magpie::System>(ctx, thisSystem, 0, [](duk_context* ctx, peterlavalle::magpie::System* thisSystem) -> duk_ret_t {

					assert(ctx == thisSystem->Host());

					thisSystem->~System();
					duk_free(ctx, thisSystem);
					// scad40::env::remove(ctx, "peterlavalle.magpie.System");
					// TODO ; find a way to actually DO THIS!?!?
				...
				// def require(name: string): bool
					scad40::push_selfie<peterlavalle::magpie::System>(ctx, thisSystem, 1, [](duk_context* ctx, peterlavalle::magpie::System* thisSystem) -> duk_ret_t {
						auto result = thisSystem->require(
							scad40::duk_string(ctx, 0)
						);
						duk_push_boolean(ctx, result ? 1 : 0);
						return 1;
				...
				// def out(message: string): void
					...
					duk_put_prop_string(ctx, idxBase, "out");

				// def err(message: string): void
					scad40::push_selfie<peterlavalle::magpie::System>(ctx, thisSystem, 1, [](duk_context* ctx, peterlavalle::magpie::System* thisSystem) -> duk_ret_t {
						thisSystem->err(
							scad40::duk_string(ctx, 0)
						);
					...
					duk_put_prop_string(ctx, idxBase, "err");

				assert(duk_get_top(ctx) == 1 + idxBase);

				// stack -> .... base .. ; [System] ;

				scad40::env::assign(ctx, "peterlavalle.magpie.System");
				// stack -> .... base .. ;

				*reinterpret_cast<duk_context**>(thisSystem) = ctx;
				new (thisSystem) peterlavalle::magpie::System();
...
inline void peterlavalle::magpie::Listener::onStart()
{
	auto ptr = reinterpret_cast<scad40::duk_ptr<Listener>*>(this);
	auto ctx = Host();
	...
	duk_get_prop_string(ctx, -1, "onStart");
	// stack -> .. base .. ; [self] ; onStart() ;
...
#ifndef _DEBUG
	duk_call_prop(ctx, base, 0);
#else
	auto result = duk_pcall_prop(ctx, base, 0);
	if (DUK_EXEC_SUCCESS != result)
	{
		const char* message = duk_safe_to_string(ctx, -1);
		std::cerr << "Failed to call `peterlavalle.magpie/Listener::onStart()` because\n\t" << message << std::endl;
		duk_error(ctx, 314, "Failed to call `peterlavalle.magpie/Listener::onStart()` because\n\t%s", message);
	}
#endif
	// stack -> .. base .. ; ?result? ;
...
inline void peterlavalle::magpie::Listener::onUpdate()
{
	auto ptr = reinterpret_cast<scad40::duk_ptr<Listener>*>(this);
	auto ctx = Host();
	...
	{
		const char* message = duk_safe_to_string(ctx, -1);
		std::cerr << "Failed to call `peterlavalle.magpie/Listener::onUpdate()` because\n\t" << message << std::endl;
		duk_error(ctx, 314, "Failed to call `peterlavalle.magpie/Listener::onUpdate()` because\n\t%s", message);
	}
#endif
	// stack -> .. base .. ; ?result? ;
...
inline void peterlavalle::magpie::Listener::onClose()
{
	auto ptr = reinterpret_cast<scad40::duk_ptr<Listener>*>(this);
	auto ctx = Host();
	...
	{
		const char* message = duk_safe_to_string(ctx, -1);
		std::cerr << "Failed to call `peterlavalle.magpie/Listener::onClose()` because\n\t" << message << std::endl;
		duk_error(ctx, 314, "Failed to call `peterlavalle.magpie/Listener::onClose()` because\n\t%s", message);
	}
#endif
	// stack -> .. base .. ; ?result? ;
...
inline scad40::duk_ptr<peterlavalle::magpie::Listener> peterlavalle::magpie::Listener::New(duk_context* ctx, const char* subclass)
{
	// stack -> ... ;
	...
	// stack -> ... ; ?object? ;
	if (!peterlavalle::magpie::Listener::As(ctx, -1))
	{
		duk_pop(ctx);
		duk_error(ctx, 314, "Thing `%s` is not usable as peterlavalle::magpie::Listener", subclass);
	}

	// stack -> ... ; object ;
	scad40::duk_ptr<peterlavalle::magpie::Listener> object(ctx, -1);
	duk_pop(ctx);
...
inline bool peterlavalle::magpie::Listener::As(duk_context* ctx, duk_idx_t idx)
{
	// stack -> ... ; idx .. base .. ;

	//
	// check each function / member ... not sure what to do about values
		// check def onStart(): void
			duk_get_prop_string(ctx, idx, "onStart");
			// stack -> ... ; idx .. base .. ; ?onStart() ;
			...
			// stack -> ... ; idx .. base .. ;
		// check def onUpdate(): void
			duk_get_prop_string(ctx, idx, "onUpdate");
			// stack -> ... ; idx .. base .. ; ?onUpdate() ;
			...
			// stack -> ... ; idx .. base .. ;
		// check def onClose(): void
			duk_get_prop_string(ctx, idx, "onClose");
			// stack -> ... ; idx .. base .. ; ?onClose() ;
...
inline scad40::duk_ptr<peterlavalle::magpie::Listener> peterlavalle::magpie::Listener::To(duk_context* ctx, duk_idx_t idx)
{
	return scad40::duk_ptr<peterlavalle::magpie::Listener>(ctx, idx);
}
#pragma endregion

#pragma region "global Time"
inline peterlavalle::magpie::Time& peterlavalle::magpie::Time::get(duk_context* ctx)
{
	auto base = duk_get_top(ctx);

	// stack -> .... base .. ;

	scad40::env::lookup(ctx, "peterlavalle.magpie.Time");
	// stack -> .... base .. ; [Time] ;

	duk_get_prop_string(ctx, base, "\xFF" "*Time");
	// stack -> .... base .. ; [Time] ; Time[*] ;

	auto ptrTime = reinterpret_cast<peterlavalle::magpie::Time*>(duk_to_pointer(ctx, -1));
	duk_pop_2(ctx);
	// stack -> .... base .. ;
...
#pragma region "global System"
inline peterlavalle::magpie::System& peterlavalle::magpie::System::get(duk_context* ctx)
{
	auto base = duk_get_top(ctx);

	// stack -> .... base .. ;

	scad40::env::lookup(ctx, "peterlavalle.magpie.System");
	// stack -> .... base .. ; [System] ;

	duk_get_prop_string(ctx, base, "\xFF" "*System");
	// stack -> .... base .. ; [System] ; System[*] ;

	auto ptrSystem = reinterpret_cast<peterlavalle::magpie::System*>(duk_to_pointer(ctx, -1));
	duk_pop_2(ctx);
	// stack -> .... base .. ;
