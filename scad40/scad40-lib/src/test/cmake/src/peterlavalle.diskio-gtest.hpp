...
void peterlavalle::diskio::Disk::subscribe(scad40::duk_string& path, scad40::duk_ptr<ChangeListener>& listener)
{
	std::stringstream log;
...
void peterlavalle::diskio::Disk::unsubscribe(scad40::duk_string& path, scad40::duk_ptr<ChangeListener>& listener)
{
	stupid_mock::get(Host()).soft_call(__FUNCTION__);
}
...
{
	stupid_mock::get(Host()).soft_call((std::string(__FUNCTION__ "(") + path.c_str() + ")").c_str());
...
void peterlavalle::diskio::Disk::foobar(scad40::duk_string& text)
{
	std::stringstream log;

	log << __FUNCTION__ << "(" << text.c_str() << ")";
